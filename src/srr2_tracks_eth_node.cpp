...
int main(int argc, char *argv[]) {
  ...
  if (!n.getParam("position", position)) {
    // Default value if position is missing
    position = "front";
  }
  ROS_INFO("ip: %s , port: %d , position: %s", ip_address.c_str(), port,
           position.c_str());
  ...
  radar_markers.marker_namespace_="srr2_" + position;
  ...
  header.frame_id = "srr2_" + position;
  ...
  while (ros::ok()) {
    ...
    tcp_interface.parse_value(&turn_signal, xcpMsgBuf, SRR2_TURN_SIGNAL_OFFSET,
                              SRR2_TURN_SIGNAL_SIZE);
    ...
    tcp_interface.parse_value(&yaw_rate_raw, xcpMsgBuf,
                              SRR2_YAW_RATE_RAW_OFFSET, SRR2_YAW_RATE_RAW_SIZE);
    tcp_interface.parse_value(&yaw_rate_raw_qf, xcpMsgBuf,
                              SRR2_YAW_RATE_RAW_QF_OFFSET,
                              SRR2_YAW_RATE_RAW_QF_SIZE);
    tcp_interface.parse_value(&yaw_rate_qf, xcpMsgBuf, SRR2_YAW_RATE_QF_OFFSET,
                              SRR2_YAW_RATE_QF_SIZE);
